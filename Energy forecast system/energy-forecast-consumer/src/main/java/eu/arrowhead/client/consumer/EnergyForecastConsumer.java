...
public class EnergyForecastConsumer extends ArrowheadConsumer {
  ...
  public static void main(String[] args) {
    new EnergyForecastConsumer(args);
  }
  ...
  private ServiceRequestForm compileSRF() {
     ...
     */
    ArrowheadService service = new ArrowheadService("EnergyForecast", Collections.singletonList("json"), metadata);
  ...
  private static void consumeService(String providerUrl) {
    ...
    final URI uri = UriBuilder.fromUri(providerUrl)
     ...
     */
    try {
      System.out.println("Requesting data for " + ts.toString());
      readout = Utility.requestEntity("GET", uri.toString(), null, Message.class);
